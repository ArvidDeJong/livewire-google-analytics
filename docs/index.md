...
- **A trait** with `trackEvent()`, `trackLead()`, `trackNewsletterSignup()` and `trackCustomEvent()`, each with an `…AfterRedirect()` variant.
- **No lost events**: what is tracked before a consent tool loads Google's tag waits and is sent once `gtag` exists, and an event tracked right before a redirect is sent on the next page.
...
- [How it works](concepts.md): the browser event, the listener, the queue and the redirect
- [Examples](examples.md): a contact form, a newsletter signup, a purchase with and without a redirect, a download
- [Testing](testing.md): assert on the event in Pest, and check it in the browser
- [Troubleshooting](troubleshooting.md): no events, double events, more events than before, missing parameters
- [FAQ](faq.md)
