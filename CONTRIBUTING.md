...
- Keep the public API compatible within 1.x: the `TracksAnalytics` trait with `trackEvent()`, `trackLead()`, `trackNewsletterSignup()` and `trackCustomEvent()`, their signatures and the event names they send (`generate_lead`, `sign_up` with `method` `newsletter`, the `ga_` prefix); the four `…AfterRedirect()` variants; the `ga:event` browser event with `name` and `params`; the `window.livewireGoogleAnalytics` object and its `queue` key; the `queue` parameter of the view; the session key `livewire-google-analytics.events` (`CarriedEvents::SESSION_KEY`) and the `id`, `name`, `params` and `at` of a stored event, because host app tests assert on them; the `sessionStorage` key `livewire-google-analytics.sent` and its content, a JSON list of ids, because a published copy of the view or the file from another version shares it; the view name `livewire-google-analytics::script`; the publish tags `livewire-google-analytics-js` and `livewire-google-analytics-views` and the path the JavaScript file is published to.
