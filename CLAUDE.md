...
- Never send a carried event twice: an `…AfterRedirect()` method must not dispatch as well (except as the fallback without a session), the view must `pull` and not `get`, and the script must check `state.carried`.
